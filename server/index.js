...
const app = express();
const powerpointDiagnostics = createPowerpointDiagnostics({ pool, salt:setupToken });
...
// Observe PowerPoint calls without storing session codes, answers or full IP addresses.
app.use(powerpointDiagnostics.middleware);
...
const presentationDiagnosticLimiter = rateLimit({ windowMs: 60 * 1000, limit: 10, standardHeaders: 'draft-8', legacyHeaders: false });

app.post('/api/presentation/diagnostics', presentationDiagnosticLimiter, asyncRoute(async (req, res) => {
  res.locals.skipAudit = true;
  const result = await powerpointDiagnostics.ingestClientEvents(req);
...
app.get('/api/superadmin/powerpoint-diagnostics', requireSuperadmin, asyncRoute(async (req, res) => {
  const result = await powerpointDiagnostics.report(req.query?.hours);
...
async function shutdown() {
  server.close(async () => {
    await powerpointDiagnostics.flushMetrics();
    await pool.end();
    process.exit(0);
  });
}
