...
function updateQuestionProgress(state) {
  ...
  if (title && /^Question\s+\d+/i.test(title.textContent.trim())) title.textContent = `Question ${position} / ${total}`;
}
...
function updatePoll(state) {
  ...
  for (const row of document.querySelectorAll('.poll-row')) {
    ...
    if (small) small.textContent = `${count} participant${count > 1 ? 's' : ''} sur ${totalParticipants}`;
  }
  const tag = document.querySelector('.poll-results')?.closest('.login')?.querySelector('.question-head .tag');
  if (tag) tag.textContent = `${totalParticipants} participant${totalParticipants > 1 ? 's' : ''}`;
}
...
const hookTimer = setInterval(() => { if (installPodiumSaveHook()) clearInterval(hookTimer); }, 300);
if (sessionCode) {
  refreshEnhancements();
